...
C_ao_iao = make_basis.get_C_ao_lo_iao(Lat, kmf, minao=minao)
...
O1_idx = list(idx_all["2 O1 2px   "]) + list(idx_all["3 O1 2px   "]) + list(idx_all["4 O1 2py   "]) + list(idx_all["5 O1 2py   "])

idx_x = get_idx_each(cell, labels=lo_labels, kind='atom nlm')
Cu_3band = list(idx_x["Cu 4s"]) + list(idx_x["Cu 3dx2-y2"])

act_idx = np.sort(Cu_3band + O1_idx, kind='mergesort')
imp_idx_fit = act_idx
