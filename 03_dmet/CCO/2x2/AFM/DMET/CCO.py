...
for iter in range(MaxIter):
    ...
    veff_ao, rdm1_glob_R = slater.get_veff_from_rdm1_emb_mpi(Lat, rhoEmb, \
            basis, comm, return_update=True)[1:]
    rdm1_glob_k = Lat.R2k(rdm1_glob_R)
    
    veff_lo = make_basis.transform_h1_to_lo(veff_ao, C_ao_lo)
    E1 = np.einsum('skpq, skqp ->', Lat.getH1(kspace=True) + veff_lo * 0.5, rdm1_glob_k) / nkpts
    E1 = E1.real
    if rank != 0:
        E1 = 0.0
    
    #veff = slater.get_veff_from_rdm1_emb_mpi(Lat, rhoEmb, basis, comm)
    ...
    rhoImp, EnergyImp, nelecImp = \
            ...
            solver=solver, solver_args=solver_args, E1=E1)
    EnergyImp *= nscsites
    ...
    if rank == 0:
        ...
        pvcor = np.zeros(vcor.length())
        dE = None
        dVcor_per_ele = None

    comm.Barrier()
    dVcor_per_ele = comm.bcast(dVcor_per_ele, root=0)
    dE = comm.bcast(dE, root=0)
    comm.Bcast(pvcor, root=0) 
    vcor.update(pvcor)
